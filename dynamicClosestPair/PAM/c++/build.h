...
//#include "../../common/pbbs/sampleSort.h"
...
struct build {
  ...
  sort_remove_duplicates(Seq const &A,
	...
	auto B = pbbs::p_quicksort(A, less);
	...
	pbbs::p_quicksort_inplace(A.slice(), less);
  ...
  sort_reduce_duplicates(Seq const &A, const Reduce& reduce) {
    ...
    auto B = pbbs::p_quicksort(A, lessE);
    t.next("sort");
